...
const files = [
  { name:'Provider KYC Documents', type:'folder', items:156, size:'—', modified:'Jul 9, 2026', icon:'📁' },
  { name:'Booking Proofs', type:'folder', items:892, size:'—', modified:'Jul 9, 2026', icon:'📁' },
  { name:'Invoice PDFs', type:'folder', items:1284, size:'—', modified:'Jul 9, 2026', icon:'📁' },
  { name:'Service Images', type:'folder', items:345, size:'—', modified:'Jul 8, 2026', icon:'📁' },
  { name:'Terms_Conditions_v3.pdf', type:'pdf', items:null, size:'245 KB', modified:'Jun 1, 2026', icon:'📄' },
  { name:'Privacy_Policy_2026.pdf', type:'pdf', items:null, size:'182 KB', modified:'Jun 1, 2026', icon:'📄' },
  { name:'Provider_Agreement.pdf', type:'pdf', items:null, size:'318 KB', modified:'Apr 10, 2026', icon:'📄' },
  { name:'platform_logo.png', type:'image', items:null, size:'48 KB', modified:'Jan 15, 2026', icon:'🖼️' },
  { name:'GST_Certificate.pdf', type:'pdf', items:null, size:'124 KB', modified:'Mar 5, 2026', icon:'📄' },
];
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:'24px', fontFamily:'Inter, sans-serif' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        ...
        <div style={{ display:'flex', gap:'10px' }}>
          <button style={{ background:'#f1f5f9', color:'#64748b', border:'none', borderRadius:'8px', padding:'9px 16px', fontWeight:'600', fontSize:'13px', cursor:'pointer' }}>📁 New Folder</button>
          <button style={{ background:'#6366f1', color:'#fff', border:'none', borderRadius:'8px', padding:'9px 16px', fontWeight:'600', fontSize:'13px', cursor:'pointer' }}>⬆️ Upload</button>
        </div>
      </div>

      {/* Stats */}
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))', gap:'16px' }}>
        {[{ l:'Total Files', v:'2,677', icon:'📂', color:'#6366f1', bg:'#eef2ff' },
          { l:'KYC Docs', v:'156', icon:'🪪', color:'#10b981', bg:'#ecfdf5' },
          { l:'Work Proofs', v:'892', icon:'📸', color:'#f59e0b', bg:'#fffbeb' },
          { l:'Storage Used', v:'4.2 GB', icon:'💾', color:'#3b82f6', bg:'#eff6ff' }].map(s => (
            ...
            <tbody>
              {filtered.map((f,i) => (
                <tr key={f.name} style={{ borderBottom:'1px solid #f8fafc', background:i%2===0?'#fff':'#fafafa' }}>
                  <td style={{ padding:'12px', fontWeight:'600', color:'#0f172a' }}>{f.icon} {f.name}</td>
